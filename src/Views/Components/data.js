...
const getByAuthor = (column, items) =>
  items.filter((candidate) => candidate.column === column);

export const columnMap = columns.reduce(
  (previous, column) => ({
    ...previous,
    [column.name]: getByAuthor(column, candidates),
  }),
  {}
);
